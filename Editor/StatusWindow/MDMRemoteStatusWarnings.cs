...
namespace Modoium.Service.Editor {
    internal class MDMRemoteStatusWarnings : VisualElement {
        ...
        public void UpdateView(MDMRemoteStatusWindow.State state) {
            if (state != MDMRemoteStatusWindow.State.CoreConnected &&
        ...
        private void updateNoContent() {
            _noContent.style.display = _statusMonitor.issues.Count == 0 ? DisplayStyle.Flex : DisplayStyle.None;
        }
        ...
        private void updateMessages() {
            ...
            foreach (var issue in _statusMonitor.issues) {
                var message = new MDMMessageBox();
                message.SetContent(MDMMessageBox.Icon.Warning, "warning message");
        ...
        private class Styles {
            public static string title = "Warnings";
            public static string bodyNoContent = "No warnings";
        }

        private class ListItemController {
            private MDMMessageBox _messageBox;

            public ListItemController(MDMMessageBox messageBox) {
                _messageBox = messageBox;
            }

            public void Update(MDMEditorStatusMonitor.Issue issue) {
                _messageBox.SetContent(MDMMessageBox.Icon.Warning, "warning message");
            }
        }
    }
}
